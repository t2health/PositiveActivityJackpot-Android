 ...
 * 
 * @author Steve Ody (stephen.ody@tee2.org) (based on Mixare edit by Justin Wetherell <phishman3579@gmail.com>)
 */
public class Marker implements Comparable<Marker> {
	...
	private float boundsTop = 0;
	private float boundsBottom = 0;
	...
	public double lon = 0f;
	
	public Marker(String name, double latitude, double longitude,
			double altitude, int color, Bitmap bitmap, String reference) {
		set(name, latitude, longitude, altitude, color, reference);
	}
	...
	public synchronized void set(String name, double latitude,
			double longitude, double altitude, int color, String reference) {
		if (name == null)
	...
	public synchronized void draw(Canvas canvas) {
		...
		try
		{
			...
			symbolXyzRelativeToCameraView.get(symbolArray);
			if (textBox == null)
				textBox = new PaintableBoxedText(textStr,18, 320, Color.WHITE,Color.argb(160,0, 0, color) , Color.WHITE);
			else
				textBox.set(textStr, 18, 320, Color.WHITE,Color.argb(160,0, 0, color) , Color.WHITE);
			if (textContainer == null)
			{
				if(column == 0)
				{
					textContainer = new PaintablePosition(textBox, 0, index * 90, 0, 1); 
					boundsLeft = 0;
					boundsRight = textBox.getWidth();
					boundsTop = index * 90;
					boundsBottom = boundsTop + textBox.getHeight();
				}
				else
				{
					textContainer = new PaintablePosition(textBox, canvas.getWidth() -textBox.getWidth(), index * 90, 0, 1); 
					boundsLeft = (canvas.getWidth() -textBox.getWidth());
					boundsRight = boundsLeft + textBox.getWidth();
					boundsTop = index * 90;
					boundsBottom = boundsTop + textBox.getHeight();
				}	
			}
			else
			{
				if(column == 0)
				{
					textContainer.set(textBox, 0, index * 90, 0, 1); 
					boundsLeft = 0;
					boundsRight = textBox.getWidth();
					boundsTop = index * 90;
					boundsBottom = boundsTop + textBox.getHeight();
				...
				{
					textContainer.set(textBox, canvas.getWidth() -textBox.getWidth(), index * 90, 0, 1); 
					boundsLeft = (canvas.getWidth() -textBox.getWidth());
					boundsRight = boundsLeft + textBox.getWidth();
					boundsTop = index * 90;
					boundsBottom = boundsTop + textBox.getHeight();
	...
	protected synchronized void drawTouchZone(Canvas canvas) {
		...
		canvas.drawRect(new RectF(boundsLeft, boundsTop, boundsRight, boundsBottom), p);
	}
